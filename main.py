...
async def get_recommendations(user_input: UserInput):
    ...
    try:
        ...
        movie_tasks = [asyncio.to_thread(fetch_movie_details, title) for title in initial_recs.recommended_movie_titles]
        book_tasks = [asyncio.to_thread(fetch_book_details, title) for title in initial_recs.recommended_book_titles]
        song_tasks = [asyncio.to_thread(fetch_song_details, title) for title in initial_recs.recommended_song_titles_and_artists]

        # Run all tasks concurrently
        movie_results, book_results, song_results = await asyncio.gather(
            asyncio.gather(*movie_tasks),
            asyncio.gather(*book_tasks),
            asyncio.gather(*song_tasks),
        )

        # STEP 3: Aggregate the results and format the final response
        print("--- Aggregating final response ---")

        # Filter out any results that had errors during fetching
        final_response = {
            "movies": [res for res in movie_results if res and not res.get("error")],
            "books": [res for res in book_results if res and not res.get("error")],
            "songs": [res for res in song_results if res and not res.get("error")],
